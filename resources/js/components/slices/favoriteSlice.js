...
const initialState = {
    favoritedFlat: localStorage.getItem("favorited")
...
const favoriteSlice = createSlice({
    ...
    reducers: {
        addToFavorite(state, action) {
            const existingIndex = state.favoritedFlat.findIndex(
            ...
            if (existingIndex >= 0) {
                ...
                localStorage.setItem(
                    "favorited",
                    JSON.stringify(state.favoritedFlat)
                );
                toast.success(
                    ` ${action.payload.area}, добавленно в избранное`,
                    {
                        position: "bottom-left",
                    }
                );
        ...
        removeFromFavorite(state, action) {
            const leaveInFavorite = state.favoritedFlat.filter(
